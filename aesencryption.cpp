...
QByteArray AESEncryption::ExpandKey(AESEncryption::Aes level, AESEncryption::Mode mode, const QByteArray &key)
{
     return AESEncryption(level, mode).expandKey(key);
}
...
QByteArray AESEncryption::expandKey(const QByteArray &key)
{
  int i, k;
